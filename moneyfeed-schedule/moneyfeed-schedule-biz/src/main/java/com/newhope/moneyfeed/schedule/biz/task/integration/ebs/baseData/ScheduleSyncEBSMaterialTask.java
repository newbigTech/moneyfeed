...
/**
 * Created by liming on 2018/7/9. 定时任务，同步EBS物料
 */
@EScheduler(jobName = "SyncEBSMaterial", shardingTotalCount = 1, cron = "0 /30 * * * ? *", overwrite = true)
@Component
public class ScheduleSyncEBSMaterialTask extends AbstractSimpleElasticJob {
	...
	public void process(JobExecutionMultipleShardingContext arg0) {
		logger.info("--------------开始同步EBS物料-----------------");

		final BaseResponse<DtoResult> resultBaseResponse = feignClient.syncEBSMaterial("ALL_ORG");

		if (!ResultCode.SUCCESS.getCode().equals(resultBaseResponse.getCode())) {
			logger.error("同步EBS物料错误", resultBaseResponse.getMsg());
		}

		logger.info("------------同步EBS物料异步线程结束-------------------");
	}
