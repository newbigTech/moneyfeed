...
JAVA_OPTION=" -Xms2048m -Xmx2048m -XX:NewRatio=2 -XX:+UseParNewGC -XX:ParallelGCThreads=2 -XX:+UseConcMarkSweepGC "
...
case "$ACTION" in
  start)
    nohup java $JAVA_OPTION -jar -Dspring.cloud.config.label=$NEWHOPE_PROFILE_GROUP $APP_HOME/ROOT.war profile=$SPRING_PROFILES >/dev/null 2>nohup.out &
    disown $!
    echo $! > "$APP_PID"
    echo "-----startup-------"
