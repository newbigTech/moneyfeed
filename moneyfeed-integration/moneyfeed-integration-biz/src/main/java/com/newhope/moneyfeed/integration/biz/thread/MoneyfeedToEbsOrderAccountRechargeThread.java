 ...
 */
public class MoneyfeedToEbsOrderAccountRechargeThread implements Runnable {

    private Logger logger = LoggerFactory.getLogger(MoneyfeedToEbsOrderAccountRechargeThread.class);

    private MoneyfeedToEbsOrderRechargeDtoReq orderRechargeDtoReq;

    private String key;
    
    public MoneyfeedToEbsOrderAccountRechargeThread(MoneyfeedToEbsOrderRechargeDtoReq orderRechargeDtoReq , String key) {
    ...
    @Override
    public void run() {
        logger.info("开始向EBS发送充值信息, EBS充值类型：" + orderRechargeDtoReq.getPayType() + " , 商城充值流水号：" + orderRechargeDtoReq.getMoneyfeedPayNo() + 
        		" ，充值金额：" + orderRechargeDtoReq.getRechargeAmount() + "，orgId：" + orderRechargeDtoReq.getOrgId() + 
        ...
        try{
	        ...
	        if(!CollectionUtils.isEmpty(ebsRechargeModelList)){
	        	...
	        	logger.error("根据商城流水号查询账户充值记录表为空：" + orderRechargeDtoReq.getMoneyfeedPayNo());
	        	return;
	        }
	        // 记录日志
	        commonService.saveFulllog(orderRechargeDtoReq.getRechargeAmount(),
	                ...
	                "收到商城账户充值的请求，商城充值流水号: " + orderRechargeDtoReq.getMoneyfeedPayNo() + "，orgId：" + orderRechargeDtoReq.getOrgId() + 
	                " ，客户编号："+ orderRechargeDtoReq.getCusNo()
	        ...
	        try {
	            EbsOrderPayOrChargeReq req = converseDtoReqToEbsReq(orderRechargeDtoReq);
	            ...
	            logger.error("请求ebs账户充值失败，商城充值流水号: " + orderRechargeDtoReq.getMoneyfeedPayNo() + "，orgId：" + orderRechargeDtoReq.getOrgId() + 
	            		" ，客户编号："+ orderRechargeDtoReq.getCusNo()
	            ...
	            if (e.getCause() != null && e.getCause() instanceof ConnectTimeoutException) {
	                ...
	                // 通知商城，订单因为网络问题，不能创建
	                logger.error("网络连接EBS系统失败, 账户充值失败, 商城充值流水号: " + 
	                			orderRechargeDtoReq.getMoneyfeedPayNo() + "，orgId：" + orderRechargeDtoReq.getOrgId() + 
	                			" ，客户编号："+ orderRechargeDtoReq.getCusNo()
	                + " ，充值金额：" + orderRechargeDtoReq.getRechargeAmount());
	        		...
	        		commonService.sendToOrderPayOrChargeMqResult(orderRechargeDtoReq.getEbsOrderNo(),
	        				...
	        				false,
	        				"请求EBS系统异常, 账户充值失败, 商城充值流水号: " + orderRechargeDtoReq.getMoneyfeedPayNo(),
	        ...
	        commonService.saveFulllog(orderRechargeDtoReq.getRechargeAmount(),
	                ...
	                "请求EBS账户充值，商城充值流水号: " + orderRechargeDtoReq.getMoneyfeedPayNo() + "，orgId：" + orderRechargeDtoReq.getOrgId() + 
	                " ，客户编号："+ orderRechargeDtoReq.getCusNo()
	                + " ，充值金额：" + orderRechargeDtoReq.getRechargeAmount(),
	                UserOperSourceType.MEDIUM_CENTER);
        }finally{
    ...
    private EbsOrderPayOrChargeReq converseDtoReqToEbsReq(MoneyfeedToEbsOrderRechargeDtoReq orderRechargeDtoReq) {
...
//        req.setDiscountAmount(discountAmount);
        ...
        req.setCustomerNo(orderRechargeDtoReq.getCusNo());
