...
public class RSmsCache {
	...
	private RedisOperCache getRedisOperCache(){
		return (RedisOperCache)smsCache;
	}
	...
	public boolean setSmsWithTimeout(String key,String value,Integer second){
		Boolean flag=Boolean.FALSE.booleanValue();
		try {
			flag=getRedisOperCache().set(key+"_sms_code",value,second);
		} catch (Exception e) {
			logger.error("设置时效缓存失败",e);
		}
		return flag;
	}
